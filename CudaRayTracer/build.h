...
__global__ void create_leaf_nodes(BVHNode* nodes, Hittable* primitive, uint32_t* d_indices, uint32_t num_primitives) {
	uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx > num_primitives - 1) return;

	nodes[idx].id = d_indices[idx];
	nodes[idx].children[0] = nullptr;
	nodes[idx].children[1] = nullptr;
	nodes[idx].bbox = primitive[d_indices[idx]].bounding_box();
}

__device__ void print_vec(vec3 v) {
	printf("%.2f, %.2f, %.2f\n", v.x, v.y, v.z);
}
